...
Mesh LoadMesh(std::vector<Vertex> &vertices, std::vector<Index> &indices,
  ...
  GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(Index),
               &indices[0], streaming ? GL_STREAM_DRAW : GL_STATIC_DRAW));
...
void DrawMesh(Mesh &mesh, float4x4 model_view_mat, float4x4 proj_mat) {
  ...
  GL_CALL(glDrawElements(GL_TRIANGLES, mesh.index_count, GL_UNSIGNED_INT,
                 (const GLvoid *)0));
}
...
int LoadImage(string_view gltf_path, string_view img_name, int tf = TF_NONE) {
  ...
  std::string img_file_path =
      gltf_path.substr(0, gltf_path.find_last_of('/') + 1) + img_name;
  // Try to find the image among already loaded images.
  for (size_t i = 0; i < images.size(); i++)
    if (images[i].path == img_file_path)
      return i;
...
Scene LoadScene(string_view gltf_path, string_view schema_path) {
  ...
  try {
    ...
    if (info->buffers()) {
      ...
      for (size_t i = 0; i < info->buffers()->size(); i++) {
        ...
        std::string bin_file_path =
            gltf_path.substr(0, gltf_path.find_last_of('/') + 1) +
            b->uri()->str();
      ...
      for (size_t i = 0; i < info->materials()->size(); i++) {
        ...
        if (m->pbrMetallicRoughness()) {
          ...
          if (pbr->baseColorTexture()) {
            int idx = info->textures()
                          ->Get(pbr->baseColorTexture()->index())
                          ->source();
    ...
    if (info->meshes()) {
      for (size_t i = 0; i < info->meshes()->size(); i++) {
        ...
        for (size_t j = 0; j < m->primitives()->size(); j++) {
          ...
          LOG_DEBUG("Primitive is " + to_string(mode == 4 ? "" : "not ") + "made out of triangles.");
          ...
          // FlatBuffers sets undefined values to 0 if undefined attributes aren't used, it's fine.
          int at_indices[4] = {
              p->attributes()->POSITION(), p->attributes()->NORMAL(),
              p->attributes()->TANGENT(), p->attributes()->TEXCOORD_0()};
          ...
          for (int at_type = 0; at_type < 4; at_type++) {
            ...
            GLfloat *v_buf = reinterpret_cast<GLfloat *>(
                &buffers[bv.buffer][bv.offset + a.offset]);
            ...
            for (int i = 0; i < count; i++) {
              ...
              case 3:
                ...
                // OpenGL UVs are vertically flipped, glTF UVs work like Vulkan UVs.
          ...
          for (int i = 0; i < num_inds; i++) {
            ...
            switch (ind_type) {
            case GL_BYTE:
              index = (fgl::Index)ind_buf[(b_stride || 1) * i];
              break;
          ...
          if (inds.size() >= 6) {
            LOG_DEBUG("Triangle 0: " + to_string(inds[0]) + " " + to_string(inds[1]) + " " + to_string(inds[2]));
            LOG_DEBUG("Triangle 1: " + to_string(inds[3]) + " " + to_string(inds[4]) + " " + to_string(inds[5]));
          }
    ...
    if (info->nodes()) {
      for (size_t i = 0; i < info->nodes()->size(); i++) {
        ...
        if (n->translation() && n->translation()->size() >= 3)
          new_node.translation = {n->translation()->Get(0),
                                 n->translation()->Get(1),
                                 n->translation()->Get(2)};
        ...
        if (n->scale() && n->scale()->size() >= 3)
          new_node.scale = {n->scale()->Get(0), n->scale()->Get(1),
                           n->scale()->Get(2)};
    ...
    LOG_ERROR("Caught exception: " + to_string(e.what()));
  }
  return new_scene;
}
...
int DrawScene(Scene scene, float4x4 model_view_mat, float4x4 proj_mat) {
  ...
  for (int r : scene.roots)
    DrawTree(r, model_view_mat, 0);
  return error;
}
