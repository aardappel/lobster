...
#define ILBASENAMES \
    ...
    F(PUSHFLD, 1) F(PUSHFLDMREF, 1) F(PUSHFLDV, 1) F(PUSHFLD2V, 1) F(PUSHFLDV2V, 1) \
    ...
    F(I2F, 0) F(A2S, 0) F(I2S, 0) F(F2S, 0) F(E2B, 0) F(E2BREF, 0) F(ST2S, 1) \
    F(RETURN, 2) \
...
#define LVALOPNAMES \
    ...
    LVAL(WRITEV) LVAL(WRITERV) LVAL(WRITEREFV) LVAL(WRITERREFV) \
