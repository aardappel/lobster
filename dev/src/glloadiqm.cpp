...
bool loadiqmmeshes(const iqmheader &hdr, uchar *buf)
{
    ...
    {
        iqmjoint &j = joints[i];
        baseframe[i] = rotationscaletrans(normalize(quat(j.rotate)), float3(j.scale), float3(j.translate));
...
bool loadiqmanims(const iqmheader &hdr, uchar *buf)
{
    ...
    for(int i = 0; i < (int)hdr.num_frames; i++)
    {
        for(int j = 0; j < (int)hdr.num_poses; j++)
        {
            ...
            auto m = rotationscaletrans(normalize(quat(&trs[3])), *(float3 *)&trs[7], *(float3 *)&trs[0]);
            if(p.parent >= 0) m = baseframe[p.parent] * m * inversebaseframe[j];
            else m = m * inversebaseframe[j];
            // this parent multiplication may have to be moved to blend time for more complicated anim features
...
Mesh *LoadIQM(const char *filename)
{
    ...
    {
        auto surf = new Surface((int *)(tris + meshes[i].first_triangle), meshes[i].num_triangles * 3);
