...
Mesh *CreatePolygon(VM &vm, Value &vl) {
    ...
    auto m = new Mesh(new Geometry(span(vbuf), "PNTC"), polymode);
    return m;
}
...
nfr("gl_rect_tc_col", "size,tc,tcsize,cols", "F}:2F}:2F}:2F}:4]", "",
    ...
    [](StackPtr &sp, VM &vm) {
        ...
        currentshader->Set();
        RenderArraySlow(PRIM_FAN, span(vb_square, 4), "PTC");
    });
...
nfr("gl_new_mesh", "format,positions,colors,normals,texcoords1,texcoords2,indices", "SF}:3]F}:4]F}:3]F}:2]F}:2]I]?", "R",
    ...
    [](StackPtr &, VM &vm, Value &format, Value &positions, Value &colors,
        ...
        auto m = new Mesh(new Geometry(span(verts, nverts * vsize), fmt, span<uint8_t>(), vsize),
                          indices.True() ? PRIM_TRIS : PRIM_POINT);
        if (idxs.size()) m->surfs.push_back(new Surface(span(idxs)));
        delete[] verts;
        return Value(vm.NewResource(m, &mesh_type));
    });
...
nfr("gl_render_tiles", "positions,tilecoords,mapsize", "F}:2]I}:2]I}:2", "",
    ...
    [](StackPtr &sp, VM &vm) {
        ...
        currentshader->Set();
        RenderArraySlow(PRIM_TRIS, span(vbuf), "pT");
    });
