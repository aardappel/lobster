...
(VM &vm, LString *title, iint xs, iint ys, iint flags, iint samples) {
    ...
    string err = SDLInit(title->strvnt(), int2(iint2(xs, ys)),
                         (InitFlags)(int)flags, max(1, (int)samples));
    if (err.empty()) {
...
TRANSLATEW(translate_fvec, "F", double, 3)
TRANSLATEW(translate_ivec, "I", iint, 2)
...
(VM &, double fovy_, double znear_, double zfar_, iint2 frame_buffer_size,
 iint2 frame_buffer_offset, iint nodepth_) {
    ...
    int2 fbs = ToVec<int2>(frame_buffer_size);
    if (fbs.x + fbs.y == 0)
        fbs = GetFrameBufferSize(GetScreenSize()) - fbo;
    auto zfar = (float)zfar_;
    auto znear = (float)znear_;
    auto fovy = (float)fovy_;
 ...
 LVector *texcoords1, LVector *texcoords2, LVector *indices) {
    ...
    for (iint i = 0; i < nverts; i++) {
        ...
        for (auto c : fmt) {
            switch (c) {
                case 'P': {
                    pos = ValueToFLT<3>(positions->AtSt(i), positions->width);
                    WriteMemInc(p, pos);
                    break;
                }
                case 'C':
                    WriteMemInc(p,
                        i < colors->len
                            ? quantizec(ValueToFLT<4>(colors->AtSt(i),
                                                      colors->width, 1))
                            : byte4_255);
                    break;
                case 'T': {
                    ...
                    WriteMemInc(p,
                        i < texcoords->len
                            ? ValueToFLT<2>(texcoords->AtSt(i), texcoords->width, 0)
                            : pos.xy());
                    texcoordn++;
                    break;
                }
                case 'N':
                    if (!normals->len) normal_offset = p - start;
                    WriteMemInc(p,
                        i < normals->len
                            ? ValueToFLT<3>(normals->AtSt(i), normals->width, 0)
                            : float3_0);
                    break;
                default: assert(0);
            }
...
(StackPtr &sp, VM &, LResource *i, LString *n) {
    ...
    int num_frames = -1;
    float framerate = 0.0;
    if (auto it = m.animations.find(string(n->strv())); it != m.animations.cend()) {
...
SETUNIFORMW(set_uniform_fvec, "F", double, float, 4)
...
SETUNIFORMW(set_uniform_ivec, "I", iint, int, 2)
SETUNIFORMW(set_uniform_ivec, "I", iint, int, 3)
SETUNIFORMW(set_uniform_ivec, "I", iint, int, 4)
#undef SETUNIFORMW
...
(VM &vm, LString *name, LVector *vec) {
    ...
    for (int i = 0; i < vec->len; i++)
        vals.push_back(ValueToFLT<4>(vec->AtSt(i), vec->width));
    gs->currentshader->Activate();
