...
#define ILNAMES \
    ...
    F(PUSHNIL) \
    F(PUSHFUN) \
    F(PUSHVAR) F(PUSHVARREF) F(LVALVAR) \
    F(PUSHIDXI) F(PUSHIDXV) F(LVALIDXI) F(LVALIDXV) \
    F(PUSHFLD) F(PUSHFLDM) F(LVALFLD) \
    F(PUSHLOC) F(LVALLOC) \
    F(BCALL) \
    F(CALL) F(CALLV) F(CALLVCOND) F(YIELD) F(CONT1) F(CONT1REF) \
    ...
    F(LOGNOT) F(LOGNOTREF) \
    F(BINAND) F(BINOR) F(XOR) F(ASL) F(ASR) F(NEG) \
    F(I2F) F(A2S) F(I2A) F(F2A) F(E2B) F(E2BREF) \
    F(JUMPFAIL) F(JUMPFAILREF) F(JUMPFAILR) F(JUMPFAILRREF) \
    F(JUMPFAILN) F(JUMPFAILNREF) \
    F(JUMPNOFAIL) F(JUMPNOFAILREF) F(JUMPNOFAILR) F(JUMPNOFAILRREF) \
    F(RETURN) \
    F(IFOR) F(IFORREF) F(SFOR) F(SFORREF) F(VFOR) F(VFORREF) \
