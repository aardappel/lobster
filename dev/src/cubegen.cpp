...
nfr("cg_load_vox", "name", "S", "R:voxels]S?",
    ...
    [](StackPtr &sp, VM &vm, Value &name) {
        ...
        if ((bufs.size() >= 8) && (strncmp((const char *)bufs.data(), "VOX ", 4) == 0)) {
                    ...
                    for (int frame = 0; frame < num_frames; ++frame) {
                        ...
                        for (int i = 0; i < dict_len; ++i) {
                            ...
                            if (key == "_r") {
                                ...
                                if (rotation != 4) {
                                    return errf(cat(".vox file uses an object rotation or flip that is not supported: ", value));
                                }
                            }
                        }
                    }
            ...
            if (voxvec->SLen() < (ssize_t)node_names.size()) return errf(".vox file uses object deduplication feature that is not supported");
