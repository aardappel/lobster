...
struct Function : Name
{
    ...
    bool multimethod;   // if false, subfunctions can be generated by type specialization as opposed to programmer implemented dynamic dispatch
    ...
    Function()                                                   :                    nargs(-1),     bytecodestart(-1), subf(NULL), sibf(NULL), multimethod(false), scopelevel(-1),  retvals(0), ncalls(0) {}
    Function(const string &_name, int _idx, int _nargs, int _sl) : Name(_name, _idx), nargs(_nargs), bytecodestart(0),  subf(NULL), sibf(NULL), multimethod(false), scopelevel(_sl), retvals(0), ncalls(0) {}
