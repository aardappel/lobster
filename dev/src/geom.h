...
template<typename T, int N> class vec
{
    ...
    bool operator<=(const vec &v) const { bool all = true; DOVEC(all = all && c[i] <= v[i]); return all; }
    bool operator< (const vec &v) const { bool all = true; DOVEC(all = all && c[i] <  v[i]); return all; }
    bool operator>=(const vec &v) const { bool all = true; DOVEC(all = all && c[i] >= v[i]); return all; }
    bool operator> (const vec &v) const { bool all = true; DOVEC(all = all && c[i] >  v[i]); return all; }

    bool operator==(const vec &v) const { bool all = true;  DOVEC(all = all && c[i] == v[i]); return all; }
    bool operator!=(const vec &v) const { bool any = false; DOVEC(any = any || c[i] != v[i]); return any; }
    ...
    string to_string() { string s = "("; DOVEC(if (i) s += ", "; s += std::to_string(c[i])); return s + ")"; }
...
template<typename T, int N> inline vec<T,N> rpow(const vec<T,N> &a, const vec<T,N> &b) { DOVECR(rpowf(a[i], b[i])); }
...
template<typename T, int C, int R> class matrix
{
    ...
    void set(int i, const V &v) { m[i] = v; } // not an operator on purpose, don't use outside this header
...
template<typename T, int C, int R> inline vec<T,R> operator*(const vec<T,R> &v, const matrix<T,C,R> &m)
{
    vec<T,R> t;
