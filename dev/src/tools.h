...
// This turns a sequence of booleans (the current value, and the values it had before) into a single number:
//   0: "became false": false, but it was true before.
//   1: "became true": true, but it was false before.
//  >1: "still true": true, and was already true. Number indicates how many times it has been true in sequence.
//  <0: "still false": false, and false before it. Negative number indicates how many times it has been false.
...
// the history in a more human readable way, and it can encode a way longer history in the same bits.
...
template<typename T> class TimeBool
{
    ...
    // It increases the counter, i.e. 2 -> 3 or -1 -> -2, indicating the amount of steps it has been true.
    // Only allows this to increase to the largest power of 2 that fits inside T, e.g. 64 and -64 for a char.
    void Advance()
    {
    ...
    // it assumes the history is "still false/true" as opposed to "became false/true" (which is also possible).
    // This is typically desirable, and usually the difference doesn't matter.
