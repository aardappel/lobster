...
void GeometryCache::RenderUnitSquare(Shader *sh, Primitive prim, bool centered) {
    if (!quadgeom[centered]) {
        ...
        quadgeom[centered] =
            new Geometry(span(centered ? vb_square_centered : vb_square, 4), "pT");
    }
    sh->Set();
...
void GeometryCache::RenderUnitCube(Shader *sh, int inside) {
    ...
    if (!cube_geom[inside]) {
        ...
        cube_geom[inside] = new Geometry(span(verts), "PNT");
        cube_ibo[inside] = GenBO(GL_ELEMENT_ARRAY_BUFFER, span(triangles));
    }
    sh->Set();
...
void GeometryCache::RenderOpenCircle(Shader *sh, int segments, float radius, float thickness) {
    ...
    if (!vibo.first) {
        ...
        vibo.first = new Geometry(span(vbuf), "P");
        vibo.second = GenBO(GL_ELEMENT_ARRAY_BUFFER, span(ibuf));
    }
    Transform(float4x4(float4(float2_1 * radius, 1)), [&]() {
        sh->Set();
