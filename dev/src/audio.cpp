...
nfr("recording_devices", "", "", "I]S]",
    "returns the available recording devices as two lists: their ids (used with recording_start) and their names",
    [](StackPtr &sp, VM &vm) {
        vector<int> ids;
        vector<string> names;
...
nfr("recording_start", "id,freq", "II", "I",
    "start recording audio from the given device id, at the given frequency. Returns a recording_id.",
    [](StackPtr &, VM &, Value id, Value freq) {
        auto ok = SDLRecordingStart(id.intval(), freq.intval());
        return Value(ok);
    });

nfr("recording_stop", "id", "I", "B",
    "stop recording. Returns false if the recording_id is invalid",
    [](StackPtr &, VM &, Value id) {
        auto ok = SDLRecordingStop(id.intval());
        return Value(ok);
    });

nfr("recording_get", "id", "I", "F]",
    "gets audio sample data in float format from the given recording device.",
