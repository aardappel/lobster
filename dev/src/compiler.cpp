...
void Compile(const char *fn, char *stringsource, vector<uchar> &bytecode,
...
bool LoadByteCode(const char *bcf, vector<uchar> &bytecode) {
    ...
    if (!bc) return false;
    if (memcmp(fileheader, bc, fileheaderlen)) {
        free(bc);
        throw string("bytecode file corrupt: ") + bcf;
    }
    uint origlen = *(uint *)(bc + fileheaderlen);
    bytecode.clear();
    WEntropyCoder<false>(bc + fileheaderlen + sizeof(uint), bclen - fileheaderlen - sizeof(uint),
                         origlen, bytecode);
    free(bc);
